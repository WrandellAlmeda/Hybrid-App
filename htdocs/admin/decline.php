...
$candidate_request = mysqli_query($conn, "SELECT * FROM candidates WHERE confirm = 3");
...
<div class="buong-box">
			...
			<h5>Declined Candidates Application</h5>
			</div>
			<div class="box-pending">
                <a class="btn btn-outline-primary first-btn" href="requests.php">Pending (<?php while ($rowUser = mysqli_fetch_array($pending)){ 
					echo $rowUser['count(*)'];
				}?>)</a>

			<a class=" btn btn-danger" href="decline.php">Delcined (<?php while ($rowDeclined = mysqli_fetch_array($declined)){ 
			...
			while($row=mysqli_fetch_array($candidate_request)){?>
				<tr>
					...
					<td class="truncate"><?php echo $row['Platform']?></td>
					...
					<div class="d-flex justify-content-between space-button">
							...
							<form action="crudFunctions.php" method="post">
								...
								<input type="submit" name="declined" value="Delete" class="btn btn-sm btn-secondary">
							</form>
							</div>					
						</div>
					</td>
